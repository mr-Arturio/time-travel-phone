  ...
  function makeCallCard(call_id, persona = "unknown") {
    ...
    `;
        ...
        const obj = {
          ...
          // changed: append final reply instead of replacing (so greeting/filler stay)
          ...
          // new: append arbitrary lines (greeting / filler)
          appendReply(text) {
            const cont = body.querySelector(".reply");
            const line = document.createElement("div");
            line.textContent = text || "";
      ...
      function handle(raw) {
        ...
        switch (type) {
          ...
          case "greet": {
            const card = ensureActiveCard(raw);
            if (card) {
              card.appendReply(`👋 ${data.text || "Hello."}`);
          ...
          case "filler_start": {
            const card = ensureActiveCard(raw);
            if (card) {
              card.appendReply(`… ${data.text || "Thinking…"}`);
              card.note("Thinking…");
            }
            break;
          }
          ...
          case "llm_done": {
            const card = ensureActiveCard(raw);
            if (card) {
              // append final reply (do not wipe greeting/filler)
          ...
          case "call_end": {
            ...
            if (card) {
              ...
              card.note(
                data.reason ? `Completed (${data.reason})` : "Completed"
              );
            }
